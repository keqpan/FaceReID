...
class JointTrainer(BaseExpRunner):
    ...
    def global_forward(self, sample, batch_idx):
        mosaic, labels = sample
        mosaic, labels = mosaic.cuda(non_blocking=True), labels.cuda(non_blocking=True)
        mosaic = mosaic/mosaic.view(mosaic.size(0),-1).max(1)[0].view(mosaic.size(0),1,1,1)
        mosaic = mosaic*255
#         denoised_imgs = linrgb_to_srgb(bilinear(mosaic))
#         denoised_imgs = denoised_imgs.clamp(0,0.6)
#         denoised_imgs = 255 * denoised_imgs/0.6

        imgs = (mosaic-127.5)/128
    ...
    #     mean_psnr = mean_psnr[mean_psnr != np.inf].mean()
        ...
        if batch_idx % 50 == 0:
            printoneline(dt(),'Te=%d TLoss=%.4f batch=%d | faceid: %.4f | gradID: %.4f gradM: %.4f' % 
                     (self.cur_epoch, self.total_loss/(batch_idx+1), batch_idx, faceid_loss, cur_grad_faceid_norm, cur_grad_arcmargin_norm))
...
#     dataset_train = torchvision.datasets.ImageFolder(train_data_dir, transform=transform)
    ...
    train_data_dir = "/home/safin/datasets/CASIA-WebFace/noised_bayer/"
...
#     noised_dataset = DemosaicDataset(train_data_dir, transform=transform)
    ...
    dataloader_train = torch.utils.data.dataloader.DataLoader(dataset_train, sampler=train_sampler, batch_size=args.batch_size, pin_memory=True, num_workers=32)
