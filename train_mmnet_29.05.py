...
class JointTrainer(BaseExpRunner):
    ...
    def global_forward(self, sample, batch_idx):
        ...
        denoised_imgs = mmnet.forward_all_iter(mosaic, M, init=False, noise_estimation=True)
        denoiser_loss_part = denoise_criterion(denoised_imgs, groundtruth)
        loss = denoiser_loss_part
        optimizer.zero_grad()
        loss.backward()
        grads = []
        for idx, p in enumerate(list(filter(lambda p: p.grad is not None, mmnet.parameters()))):
