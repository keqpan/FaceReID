...
class JointTrainer(BaseExpRunner):
    ...
    def global_forward(self, sample, batch_idx):
        mosaic, groundtruth, _ = sample
        mosaic, groundtruth = mosaic.cuda(non_blocking=True), groundtruth.cuda(non_blocking=True)

        noised = linrgb_to_srgb(bilinear(mosaic)/255)/0.6
        groundtruth = linrgb_to_srgb(groundtruth/255)/0.6
        
        noise_out = denoiser(noised)
        noise = noised - groundtruth 
        
        denoiser_loss_part = denoise_criterion(noise_out, noise)/mosaic.size(0)
        loss = denoiser_loss_part
        optimizer.zero_grad()
        loss.backward()
        grads = []
...
#         denoised_imgs, cur_grad_dn_norm, denoiser_loss = runner.train(mosaic, M, groundtruth, init=False, noise_estimation=True)
    ...
    #     mean_psnr = mean_psnr[mean_psnr != np.inf].mean()
        denoiser_loss = float(denoiser_loss_part)
...
if __name__ == '__main__':
    ...
    train_sampler = SubsetRandomSampler(train_indices[:50000])
...
#     dataset_train = torchvision.datasets.ImageFolder(train_data_dir, transform=transform)
    ...
    denoise_criterion = nn.MSELoss(size_average=False).cuda() #nn.L1Loss().cuda()
...
#     optimized_params = itertools.chain(faceid.parameters(), arcmargin.parameters())
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=lr_milestones, gamma=0.2)
...
#     arcmargin_ckpt_path = "/home/safin/FaceReID/ckpt/mobilefacenet_08.05/arcmargin/weights_60"
    models_dict = {
                    'dncnn': {
                        'model': denoiser,
#                         'load_ckpt': "/home/safin/FaceReID/ckpt/mmnet_5it_26.05/mmnet/weights_20"
                    },
#                     'faceid': {
#                          'model': faceid,
