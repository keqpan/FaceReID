...
from loss import AngleLoss, PSNR
...
def global_forward(sample, batch_idx, optimizer, total, correct, total_loss, train_loss_arr):
    ...
    faceid_inputs = (denoised_imgs*255 - 127.5)/128
    # compute output

    outputs, *relu_feat = faceid(faceid_inputs)
    _, *relu_feat_gt = faceid((groundtruth*255 - 127.5)/128)
    ...
    fw_3 = 0
    fw_4 = 3/7
    feat_loss = 6e-4 * (fw_0 * denoise_criterion(relu_feat_gt[0], relu_feat[0]) +
                        fw_1 * denoise_criterion(relu_feat_gt[1], relu_feat[1]) +
                        fw_2 * denoise_criterion(relu_feat_gt[2], relu_feat[2]) +
                        fw_3 * denoise_criterion(relu_feat_gt[3], relu_feat[3]) +
                        fw_4 * denoise_criterion(relu_feat_gt[4], relu_feat[4]))
    loss = faceid_loss + feat_loss
    cur_psnr = PSNR(denoised_imgs, groundtruth, PIXEL_MAX=1.).mean().data.cpu().numpy().item()
...
#     cur_grad_faceid_norm = np.sum(grads)
    ...
    printoneline(dt(),'Te=%d TLoss=%.4f batch=%d | acc: %.4f%% faceid: %.4f L1: %.4f fL: %.4f | gradDN: %.4f psnr: %.4f' % 
                 (epoch, total_loss/(batch_idx+1), batch_idx, 100. * correct/total, faceid_loss.data.cpu().numpy().item(), 
                 denoiser_loss.data.cpu().numpy().item(), feat_loss.data.cpu().numpy().item(), cur_grad_dn_norm, cur_psnr))

    return loss, total, correct, total_loss
        
def run_epoch(dataloader, optimizer):
    total = 0
    correct = 0
    train_loss_arr = []
    total_loss = 0
    ...
    return total, correct, total_loss, train_loss_arr
...
if __name__ == '__main__':
    ...
    noised_dataset = NoisedDataset(train_data_dir, transform=transform, noise_transform=GaussianNoise(std=high_noise_std_arr, threshold=0.7, clamp=[0,1]))
...
#                     shortcut=(False,True))
    ...
    n_ckpt = 30
#     denoiser_ckpt_path = "/home/safin/ms-thesis/ckpt/joint_dncnn_13.03/denoiser/weights_1"
#     denoiser_ckpt_path = "/home/safin/ckpt/1st_udnetpa/weigths_"+str(n_ckpt)
#     denoiser_ckpt_path = "/home/safin/ms-thesis/ckpt/joint_09.03_int_feat/denoiser/weights_5"
#     denoiser.load_state_dict(torch.load(denoiser_ckpt_path))
#     freeze_model(denoiser)
    denoiser = denoiser.cuda()
    
    faceid = sphere20a(return_features=True)
...
#             optimized_params = itertools.chain(denoiser.parameters(), faceid.parameters())
            optimized_params = denoiser.parameters()
            optimizer = optim.SGD(optimized_params, lr=lr, momentum=0.9, weight_decay=5e-4)
        total, correct, total_loss, train_loss_arr = run_epoch(dataloader_train, optimizer)
        ...
        total, correct, total_loss, train_loss_arr = run_epoch(dataloader_val, None)
        print("\n")
