...
#     _, *relu_feat_gt = faceid((groundtruth*255 - 127.5)/128)
    denoiser_loss = denoise_criterion(denoised_imgs, groundtruth)/(3*labels.size(0))
    faceid_loss = faceid_criterion(outputs, labels)
    faceid_w = 1
    denoiser_w = 0
    loss = faceid_w*faceid_loss + denoiser_w*denoiser_loss
...
#     cur_psnr = 10*torch.log10(denoiser_loss/(112*96)).data.cpu().numpy().item()
    
    printoneline(dt(),'Te=%d TLoss=%.4f batch=%d | acc: %.4f%% faceid: %.4f denoise: %.4f | gradDN: %.4f psnr: %.4f' % 
                 (epoch, total_loss/(batch_idx+1), batch_idx, 100. * correct/total, faceid_loss.data.cpu().numpy().item(), 
                 denoiser_loss.data.cpu().numpy().item(), cur_grad_dn_norm, cur_psnr))
...
#     denoise_criterion = nn.L1Loss().cuda()
    denoise_criterion = nn.MSELoss(reduction="sum").cuda()
