...
num_avg_batches = 2
def global_forward(sample, batch_idx, optimizer, total, correct, total_loss, train_loss_arr, denoiser_loss_arr, faceid_loss_arr):
...
#     mean_psnr = mean_psnr[mean_psnr != np.inf].mean()
    ...
    denoiser_loss_arr.append(denoiser_loss)
    faceid_loss_arr.append(faceid_loss)
    total_loss += cur_loss
...
def train_epoch(dataloader, optimizer, total, correct, total_loss, train_loss_arr, denoiser_loss_arr, faceid_loss_arr):
    for batch_idx, sample in enumerate(dataloader):
        if stop_flag:
            break
        total_loss = global_forward(sample, batch_idx, optimizer, total, correct, total_loss, train_loss_arr, denoiser_loss_arr, faceid_loss_arr)
...
#     denoise_criterion = nn.L1Loss().cuda()
    ...
    total_denoiser_loss_arr = []
    total_faceid_loss_arr = []
    total_train_acc_arr = []
